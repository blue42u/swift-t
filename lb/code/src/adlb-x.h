...
adlb_code ADLBX_Init(int nservers, int ntypes, int type_vect[],
                    int *am_server, MPI_Comm adlb_comm,
                    MPI_Comm *worker_comm);

adlb_code ADLBX_Server(long max_memory);

adlb_code ADLBX_Version(version* output);

MPI_Comm ADLBX_GetComm_workers(void);

MPI_Comm ADLBX_GetComm_leaders(void);

adlb_code ADLBX_Hostmap_stats(unsigned int* count,
                             unsigned int* name_max);

adlb_code ADLBX_Hostmap_lookup(const char* name, int count,
 ...
 */
adlb_code ADLBX_Hostmap_list(char* output, unsigned int max,
 ...
 */
adlb_code ADLBX_Put(const void* payload, int length, int target, int answer,
                   int type, adlb_put_opts opts);

/*
  Put a data-dependent task into the global task queue.  The task will
  be released and eligible to be matched to an ADLBX_Get call once all
 ...
 */
adlb_code ADLBX_Get(int type_requested, void** payload,
...
*/
adlb_code ADLBX_Iget(int type_requested, void* payload, int* length,
                    int* answer, int* type_recvd, MPI_Comm* comm);
/*
 ...
 */
adlb_code ADLBX_Aget(int type_requested, adlb_payload_buf payload,
                     adlb_get_req *req);
 ...
 */
adlb_code ADLBX_Amget(int type_requested, int nreqs, bool wait,
 ...
 */
int ADLBX_Locate(adlb_datum_id id);
...
adlb_code ADLBX_Create(adlb_datum_id id, adlb_data_type type,
...
adlb_code ADLBX_Create_integer(adlb_datum_id id, adlb_create_props props,
                              adlb_datum_id *new_id);

adlb_code ADLBX_Create_float(adlb_datum_id id, adlb_create_props props,
                              adlb_datum_id *new_id);

adlb_code ADLBX_Create_string(adlb_datum_id id, adlb_create_props props,
                              adlb_datum_id *new_id);

adlb_code ADLBX_Create_blob(adlb_datum_id id, adlb_create_props props,
                              adlb_datum_id *new_id);
...
adlb_code ADLBX_Create_container(adlb_datum_id id,
 ...
 */
adlb_code ADLBX_Add_dsym(adlb_dsym symbol, adlb_dsym_data data);
...
adlb_code ADLBX_Exists(adlb_datum_id id, adlb_subscript subscript, bool* result,
                       adlb_refc decr);
 ...
 */
adlb_code ADLBX_Store(adlb_datum_id id, adlb_subscript subscript,
 ...
 */
adlb_code ADLBX_Retrieve(adlb_datum_id id, adlb_subscript subscript,
 ...
 */
adlb_code ADLBX_Enumerate(adlb_datum_id container_id,
...
adlb_code ADLBX_Read_refcount_enable(void);

adlb_code ADLBX_Refcount_incr(adlb_datum_id id, adlb_refc change);
 ...
 */
adlb_code ADLBX_Subscribe(adlb_datum_id id, adlb_subscript subscript,
...
adlb_code ADLBX_Container_size(adlb_datum_id container_id, int* size,
                              adlb_refc decr);

adlb_code ADLBX_Lock(adlb_datum_id id, bool* result);

adlb_code ADLBX_Unlock(adlb_datum_id id);

/**
  Get information about a type based on name.
  Returns error if not found
 */
adlb_code ADLBX_Data_string_totype(const char* type_string,
              adlb_data_type* type, adlb_type_extra *extra);

const char *ADLBX_Data_type_tostring(adlb_data_type type);

/*
  Convert string to placement enum value.
  Case insensitive.
 */
adlb_code ADLBX_string_to_placement(const char *string,
                           adlb_placement *placement);

adlb_code ADLBX_Server_idle(int rank, int64_t check_attempt, bool* result,
                 int *request_counts, int *untargeted_work_counts);

adlb_code ADLBX_Finalize(void);

/**
   Tell server to fail.
 */
adlb_code ADLBX_Fail(int code);

void ADLBX_Abort(int code);
